...
<body>
    ...
    $query = "select nome, pk_id_jogo from jogos where nome = '" . $nome . "';";
    $result =  $con->executQuery($query);
    $dados = mysqli_fetch_assoc($result);

    if (mysqli_num_rows($result) != 0) {

        if($id != $dados['pk_id_jogo']){
            $_SESSION['nomeJaRegistrado'] = true;
            $con->fecharConexao();
            header('location: jogo-altera2.php');
            exit();
    ...
    $query = "update jogos set nome = '$nome', preco = '$preco', requisitos = '$req', descricao = '$desc', sistema = '$sys', data_lancamento = '$data' where pk_id_jogo = $id;";
    ...
    if (!empty($_FILES['arquivo']['name'])) {
        ...
        if ($extensao != "png") {
            ...
            $con->fecharConexao();
            header('location: jogo-alterar2.php');
            exit();
        }
    ...
    <h1> Jogo alterado com sucesso! </h1>
